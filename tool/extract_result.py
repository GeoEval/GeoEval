...
def extract_answer(response, quick_extract=False):
    ...
    if quick_extract: # if quick can get answer, use quick
        ...
        try:
            result = re.search(r'answer is (\w)\.</s>', response)
            if result:
...
if __name__ == '__main__':
    ...
    for problem_id, data_item in tqdm(results.items()):
        try:
            extraction  = extract_answer(data_item["solution"][args.model_name]["response"], args.quick_extract)
            # import pdb; pdb.set_trace()
            response = data_item["solution"][args.model_name]
            data_item["solution"][args.model_name]["extraction"] = extraction
        except:
            pass
