...
from .prompt import convert_to_llama2_input_format, convert_to_general_input_format, convert_to_easy_input_format, convert_to_choice_input_format, convert_to_general_choice_input_format
...
class GeoApiEvalDataset(Dataset):
    ...
    def _process_per_instance(self, instance: dict, merge_type: str, prompt_type: str) -> dict:
        ...
        if (not self.args.backward_reason) and (not self.args.aug_reason) and (not self.args.solid_reason):
            example = naive_merge(
        ...
        elif self.args.backward_reason:
            # import pdb; pdb.set_trace()
            example = naive_merge(
                diagram_description=instance["diagram_description"],
                text=instance["backward_text"],
                choice_list=None,
                aug=False,
                back=True
            )
        elif self.args.aug_reason:
            example = naive_merge(
                diagram_description=instance["diagram_description"],
                text=instance["text"],
                choice_list=instance["choice_list"],
        ...
        elif prompt_type == "general_choice":
            example = convert_to_general_choice_input_format(example)
        else:
...
class GeoEvalDataset(Dataset):
    ...
    def _process_per_instance(self, instance: dict, merge_type: str, prompt_type: str) -> dict:
        ...
        if (not self.args.backward_reason) and (not self.args.aug_reason):
            example = naive_merge(
                diagram_description=instance["diagram_description"],
                text=instance["text"],
                choice_list=instance["choice_list"],
            ...
            example = naive_merge(
                diagram_description=instance["backward_semantic"],
                text=instance["backward_text"],
                choice_list=None,
                aug=False,
                back=True
